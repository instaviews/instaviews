    ...
    var intid = setInterval(function() 
        {
            ...
            if(myC > 5) {
                fs.readFile(sandbox.path + sandbox.folder + "/logfile.txt", "utf8", (err, data) => {
                    if (data) {
                        console.log('LogFile has data!', data);
                        console.log("DONE");
            ...
            //This check should be all we need. If the script.sh is correctly creating a completed file every time,
            //but for some reason it is not right now
